...
| `parent_record_id` | `INT` | FK→cost_records, NULL | 父记录（历史兼容字段；早期赊账→还款关联使用，当前还款不再新增子账单） |
...
**赊账与还款建模规则**：
- 赊账不单独建表，属于 `cost_records` 的一种结算状态。采购赊账使用 `record_type=cost`，销售未收款使用 `record_type=income`，并设置 `record_subtype=赊账`、`counterparty`、`due_date`。
- `amount` 是业务发生额，计入成本/收入和利润；`settled_amount` 是已结算金额；`unsettled_amount = amount - settled_amount`；`settlement_status` 由两者派生为 `unsettled` / `partial` / `settled`。
- 还款/收款不新增经营账单。`POST /api/v1/debts/settle` 与 `settle_debt` 只更新原赊账记录的 `settled_amount`、`settlement_status`、`settled_at`。
- 历史兼容：若存在 `record_type=income`、`category=还款` 且 `parent_record_id` 指向原赊账的记录，汇总、利润、分类统计、报表必须排除，避免把还款重复算作收入。
- 如未来需要展示还款流水，应新增 settlement event/audit 结构或使用审计日志；不要把还款作为 `cost_records` 的收入/支出记录。
