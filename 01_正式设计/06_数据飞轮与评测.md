...
- 🚧 LLM 预标注 API（待落地）
- 🚧 Daily Review Inbox（待落地：按风险 session 分组，而不是默认平铺全部 turn）
- 🚧 ReviewIssueChain（待落地：trigger/context/result turns、expected behavior、chain 状态机）
- 🚧 判断流程优先审核面板（待落地：诊断、证据 checklist、root cause、expected、闭环出口）
